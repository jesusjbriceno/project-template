...
| 5 | Strongly typed IDs | **Implemented (hardened).** `sealed class` + `IEquatable<T>` value objects wrapping `Guid` with private constructors and `New()`/`From(Guid)` factories plus implicit `Guid` conversion. Prevents invalid IDs via `default(T)`, `new(Guid.Empty)`, or public positional constructors. IDs in `ValueObjects/Ids/`. Domain BCL-only; Infrastructure maps in Phase 3 |
...
| 9 | RefreshToken family revoke | `FamilyId` per token; `IsActive/IsExpired/IsReuseSignal/Rotate/Revoke`. `IsReuseSignal()` = any revoked token presented again. Domain returns family id; Application cascades |
...
| Aspect | Cost | Benefit |
|---|---|---|
| Boilerplate | ~7 ID classes + tests before any entity compiles | Compile-time prevention of `Guid` swaps (e.g. `RemoveRole(roleId: x, userId: y)` won't compile) |
...
Cost is one-time at Phase 2 start. `Guid` remains the wire/storage format — the sealed class is a typed wrapper, not a different identifier.
...
Simple IDs (`UserId`, `RoleId`, `PermissionId`, `RefreshTokenId`, `MenuItemId`) use `sealed class` + `IEquatable<T>` with private constructors — no public positional constructor, no `default(T)` bypass. Junction IDs (`UserRoleId`, `RolePermissionId`) compose two FK IDs via `From(UserId, RoleId)` factory with null guards. `RefreshToken.FamilyId` stays a raw `Guid` (opaque linkage, not a domain ID).

## Testing Strategy

| Layer | What | How |
|---|---|---|
| Unit (Domain) | ID primitives | xUnit `[Fact]`: `New()` unique, `From(Guid)` round-trips, equality value-based, implicit `Guid` conversion identity, `default(T)` null, composite null-guards, `Deconstruct` destructuring |
...
**TDD order (revised)**: ID classes → Email → PermissionKey → DeletionPolicy → Permission → Role → RolePermission → User → UserRole → RefreshToken → MenuItem → exceptions. ID tests are the first commit. EF Core conversion tests land with Phase 3 Infrastructure.
...
- ~~Strongly typed IDs?~~ → **Resolved:** Phase 2. Junction IDs use a composite-of-FKs sealed class — confirm with the team whether they prefer a single synthetic `Guid` per junction row.
